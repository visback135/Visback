...
export default function VisbackDashboard() {
  ...
  const handleLogin = (e?: React.SyntheticEvent) => {
    ...
    if (encontrado.estado === 'pendiente' && encontrado.rol !== 'admin') {
  ...
  const cambiarEstadoUsuario = (email: string) => {
    setUsuariosRegistrados(prev => prev.map(u => {
      if (u.email === email) {
        return { ...u, estado: u.estado === 'activo' ? 'pendiente' : 'activo' };
      }
      return u;
    }));
  };
                    ...
                    <tbody className="divide-y">
                      {usuariosRegistrados.map((u, i) => (
                        <tr key={i}>
                          ...
                          <td className="p-3 text-center">
                            {u.rol !== 'admin' && (
                              <button onClick={() => cambiarEstadoUsuario(u.email)} className={`px-3 py-1 rounded-lg text-xs font-bold cursor-pointer transition-all ${u.estado === 'activo' ? 'bg-red-100 text-red-600 hover:bg-red-200' : 'bg-emerald-600 text-white hover:bg-emerald-700'}`}>
